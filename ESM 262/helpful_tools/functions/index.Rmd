...
possible_cars4 = data.frame(mass=c(31000,45000,38000, 35000), area = c(25,30,22, 17))

possible_cars4$power = autopower(V=speed_base, A = possible_cars$area, m=possible_cars$mass)
...
res = sapply(cdrag,  autopower, A=results$area, V=results$speed, m=results$mass, crolling=0.15, pair=1.2,g=9.8)
# cdrag = sequence, autopower = functions, rest are the variables! it will calculate those. 
# res = results!
# applying the function to these columns. This will repeat for every drag coefficient in the function. 
# Sapply allows you to do tricky sequences for the different values. 
...
# Whatever you call your function, that;s the first thing lsited, then the inputs, then the parameters. 

res_tmp = sapply(tmp,  autopower, A=results$area, V=results$speed, m=results$mass, crolling=0.15, pair=1.2,g=9.8)
# Will now give you 3 columns, becuase you only gave it 3 inputs in tmp earlier.
